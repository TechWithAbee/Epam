...
def check(row_start: int, row_end: int, column_start: int, column_end: int) -> List[List[int]]:
    full_list = []
    for i in range(row_start, row_end + 1): # i = 2
        part_list = []
        for j in range(column_start, column_end + 1): # j = 3
            part_list.append(i * j) # 2 * 3 = 6
        full_list.append(part_list)
    return full_list

# Usage
print(check(2, 4, 3, 7)) # [[6, 8, 10, 12, 14], [9, 12, 15, 18, 21], [12, 16, 20, 24, 28]]